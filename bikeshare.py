...
def preference(prompt, preferences=('y', 'n')):
    """Returns a valid input from the user given an array of possible answers.
    """

    while True:
        preference = input(prompt).lower().strip()
        # terminate the program if the input is end
        if preference == 'end':
            raise SystemExit
        # pops up if the input has only one name
        elif ',' not in preference:
            if preference in preferences:
                break
        # pops up if the input has more than one name
        elif ',' in preference:
            preference = [i.strip().lower() for i in preference.split(',')]
            if list(filter(lambda x: x in preferences, preference)) == preference:
                break

        prompt = ("\nSomething is wrong. Please kindly check the formatting and "
...
def get_filters():
    """Ask the user to indicate city(ies) and filters, month(s) and weekday(s).
    Returns:
    ...
    while True:
        ...
        month = preference("\nFrom January to June, for what month(s) do you "
                       "want do filter data? Use commas to list months.\n>",
                       months)
        day = preference("\nFor what weekday(s) do you want do filter bikeshare "
                     "data? Use commas to list days.\n>", weekdays)

        # to confirm user's preference
...
def load_data(city, month, day):
    ...
    print("\nThe program is loading the data for the filters of your preference.")
...
def station_stats(df):
    ...
    print("For the selected filters, the most common end station: " +
...
def user_stats(df, city):
    ...
    try:
        ...
        print("ooops! There is no data of user genders for {}."
              .format(city.title()))
    ...
    try:
        ...
        print("ooops! There is no data of birth year for {}."
              .format(city.title()))
...
def raw_data(df, mark_place):
    ...
    while True:
        for i in range(mark_place, len(df.index)):
            ...
            if preference("Do you want to keep printing raw data?"
                      "\n\n[y]Yes\n[n]No\n\n>") == 'y':
                continue
            else:
                break
